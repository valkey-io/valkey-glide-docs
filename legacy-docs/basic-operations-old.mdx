...
By the end of this section we will cover some basic Valkey commands.

## SET and GET

SET, GET can be used to update a single value at a time.
...
Using [MSET](https://valkey.io/commands/mset/) and [MGET](https://valkey.io/commands/mget/), you can perform operations in batch.
...
When using SET, you can specify an expiration time after which the value will be removed.
...
To set expiration on existing keys use EXPIRE.

You can also use PERSIST to remove the expriation time.
