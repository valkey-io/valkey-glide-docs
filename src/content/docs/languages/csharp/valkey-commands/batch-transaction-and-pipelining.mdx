...
var pipeline = new ClusterBatch(isAtomic: false);
pipeline.StringSetAsync("key", "value");                          // OK
pipeline.ListLeftPop("key");                                      // WRONGTYPE error (not a list)
pipeline.KeyRename("{non-existing-key}", "{non-existing-key}:1"); // NO SUCH KEY error
var res = await clusterClient.Exec(pipeline, raiseOnError: false);

// res is an object?[] with one entry per command:
//   res[0] = true                            (StringSet succeeded)
//   res[1] = RequestException("WRONGTYPE")   (ListLeftPop failed)
//   res[2] = RequestException("no such key") (KeyRename failed)
```

```csharp
// Transaction with raiseOnError = true
var transaction = new Batch(isAtomic: true);
transaction.StringSetAsync("key", "value");  // OK
transaction.ListLeftPop("key");              // WRONGTYPE error (not a list)
transaction.StringGetAsync("key");           // Would execute, but error is raised first
try
{
    var res = await client.Exec(transaction, raiseOnError: true);
}
catch (RequestException ex)
{
    // ex.Message: "WRONGTYPE: Operation against a key holding the wrong kind of value"
}
```
...
// Create atomic batch
var transaction = new Batch(isAtomic: true);
transaction.StringSetAsync("account:source", "100");
transaction.StringSetAsync("account:dest", "0");
transaction.StringIncrementAsync("account:dest", 50);
transaction.StringDecrementAsync("account:source", 50);
transaction.StringGetAsync("account:source");

// Execute with raiseOnError = true
var res = await client.Exec(transaction, raiseOnError: true);
// Results: [true, true, 50, 50, "50"]
```
...
var res = await client.Exec(pipeline, raiseOnError: true);
...
var transaction = new ClusterBatch(isAtomic: true);
transaction.StringSetAsync("{user:100}:visits", "1");
transaction.StringIncrementAsync("{user:100}:visits", 5);
transaction.StringGetAsync("{user:100}:visits");
...
var res = await client.Exec(pipeline, raiseOnError: false, options);
// Results: [true, 125, "125", 1, 2, ["user2", "user1"]]
```
