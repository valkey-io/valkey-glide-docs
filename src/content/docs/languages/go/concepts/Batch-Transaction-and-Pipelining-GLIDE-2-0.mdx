...
For **cluster** (cluster mode enabled) clients (Mirrors `StandaloneBatch` but routes commands based on slot ownership, splitting into sub-pipelines if needed, [Read more in Multi-Node support](https://github.com/valkey-io/valkey-glide/wiki/Golang-Wrapper#multi-node-support)).
...
<Aside>
  When `isAtomic = true`, all keys in the `ClusterBatch` must map to the same slot. Attempting to include keys from different slots will result in an exception. Read more in [Multi-Node support](https://github.com/valkey-io/valkey-glide/wiki/Golang-Wrapper#multi-node-support).
...
* **Retry on Connection Errors**
  * If a connection error occurs, the entire batch (or sub-pipeline, [Read more in Multi-Node support](https://github.com/valkey-io/valkey-glide/wiki/Golang-Wrapper#multi-node-support))  is retried from the start.
