...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Php">
    PHP uses native `string` type for all Valkey string operations. PHP strings are binary-safe and can contain arbitrary bytes, including null bytes and non-UTF-8 data.
    ...
    // Working with hex encoding for binary data
    $hexValue = bin2hex($binaryValue);  // Convert to hex string
    $binaryData = hex2bin($hexValue);   // Convert back to binary
    ```

    #### Binary Safety

    * PHP strings are binary-safe by default and can store any byte sequence
    * No special type or conversion needed for binary data
    * Use `bin2hex()` and `hex2bin()` for hex encoding/decoding
    * Use `base64_encode()` and `base64_decode()` for base64 encoding/decoding
  </TabItem>
</Tabs>
