...
- **State reset on release:** When a client is returned, the pool sends `DISCARD` and `UNWATCH` only if the connection has dirty state (e.g., an active `MULTI` or `WATCH`). Clean connections are returned directly to the idle set with no round-trip.
