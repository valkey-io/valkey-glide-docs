...
Client Pool is in preview. The API may change in a future release without following normal semantic versioning guarantees, and known limitations are listed below.
</Aside>
...
| `request_timeout_ms` / `requestTimeout` / `RequestTimeout` | duration | 5 sec | Timeout for state reset (`DISCARD`, `SELECT`) when returning a client |
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Node">
    ...
    const pool = await ClientPool.create(clientConfig, {
        maxSize: 10,
        minIdle: 2,
        acquireTimeoutS: 5,
    });
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Java">
    ...
    // client is automatically returned to the pool on close
    ```
  </TabItem>

  <TabItem label="Node">
    Using `borrow()` (recommended):
...
- **No per-client callbacks:** A pooled client config cannot use a custom IAM credentials provider (Java) or a custom address resolver (Java and Go). These are language-side callbacks the pool cannot forward to the connections it manages, so pool creation fails fast rather than silently signing with the default credential chain or connecting to an unresolved address. Resolve addresses ahead of time, and configure IAM without a custom provider.
- **No background eviction:** Idle clients are evicted lazily when `acquire()` is called, not on a background timer.
- **State reset is unconditional:** When a client is returned, the pool always sends `DISCARD` followed by `SELECT <configured database>`, even if the connection was never mutated. `DISCARD` also cancels any active `WATCH`. Skipping the round-trip for provably clean connections is planned but not yet implemented.
- **Scopes do not outlive the pool:** Closing the pool invalidates any isolated scope still open on a borrowed client. Commands on such a scope fail rather than continuing to run on a connection the pool no longer owns.
