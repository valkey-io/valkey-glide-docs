...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Php">
    :::note
    PHP GLIDE uses a synchronous blocking API, which means only one request can be in-flight at a time. 
    Therefore, the inflight request limit configuration is not applicable and is not exposed in the PHP API.
    :::
  </TabItem>
</Tabs>
