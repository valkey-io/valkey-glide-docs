...
When using the AZ Affinity All Nodes read strategy, the `clientAz` setting is required to ensure that readonly commands are spread equally across all nodes (primary and replicas) within the specified availability zone if they exist. Unlike AZ Affinity Replicas and Primary, the local primary is an equal member of the rotation rather than a fallback, which evens out read load when the number of replicas per availability zone is limited.
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Java">
    :::note[Coming soon]
    The AZ Affinity All Nodes read strategy is not yet available in the Java client.
    :::
  </TabItem>

  <TabItem label="Node">
    :::note[Coming soon]
    The AZ Affinity All Nodes read strategy is not yet available in the Node client.
    :::
  </TabItem>

  <TabItem label="Go">
    :::note[Coming soon]
    The AZ Affinity All Nodes read strategy is not yet available in the Go client.
    :::
  </TabItem>
