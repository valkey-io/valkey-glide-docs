...
Currently, GLIDE does not support direct integration with third-party logging libraries. While there are interests in adding this capability, 
...
If the file name is a relative path `./client_logs`, the logs will be written inside the foler `glide-logs`. For example, `./glide-logs/client_logs.<timestamp>`.
...
To disable these color codes, you can use set the environment variable `NO_COLOR=1` before starting the logger. 