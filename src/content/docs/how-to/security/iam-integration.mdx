...
* When a refreshed token is available, GLIDE **re-authenticates each connection lazily** — the new token is applied on the next command sent over that connection, not immediately pushed to all connections
* If a connection is idle during a node failover or reconnection event, GLIDE also refreshes the IAM token **during reconnection** to ensure a valid token is used even without active commands
* Each connection session remains valid for up to **12 hours**, after which GLIDE handles re-authentication automatically
* GLIDE handles all **token management**, **refresh operations**, and **re-authentication** behind the scenes
* Manual token refresh is available via **`refreshIamToken`** (see [Manual Refresh](#manual-refresh) below)
...
You can call `refreshIamToken` (or language equivalent) to immediately request a fresh token from AWS and update the internal token cache.

:::note[What this does and does not do]
`refreshIamToken` **generates a new token and caches it**. It does **not** immediately re-authenticate existing connections — each connection will use the new token lazily, on its next command.
