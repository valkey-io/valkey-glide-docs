...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Java">
    ...
    * Multiple certificates can be provided by concatenating them in PEM format
    ...
    byte[] rootCert = "-----BEGIN CERTIFICATE-----MIIDXTCCAkWg...".getBytes(StandardCharsets.UTF_8);
  ...
  <TabItem label="Node">
    ...
    * Multiple certificates can be provided by concatenating them in PEM format
    ...
    const rootCertificates = Buffer.from("-----BEGIN CERTIFICATE-----MIIDXTCCAkWg...");
  ...
  <TabItem label="Go">
    ...
    * `config.LoadRootCertificatesFromFile` reads a PEM file and returns the bytes, ready to pass to `WithRootCertificates`
    ...
    func ConnectStandaloneWithCustomRootCABytes() error {
        rootCerts := []byte("-----BEGIN CERTIFICATE-----MIIDXTCCAkWg...")
    ...
    * Multiple certificates can be provided as PEM strings via the `root_certs` array, or by pointing `ca_path` at a directory of `.crt`/`.pem` files
    ...
    client = Valkey.new(
      ...
      ssl_params: {
        ...
        cert: "/etc/glide/client.pem",
        key: "/etc/glide/client.key"
      }
    )
    ```
  </TabItem>
...
mTLS still requires TLS to be enabled on the top-level client configuration (`use_tls=True` in Python, `.useTLS(true)` in Java, `useTLS: true` in Node.js, `.WithUseTLS(true)` in Go, or `use_tls: true` in PHP).
The advanced TLS configuration only supplies the client-side material; it does not enable TLS on its own.
In PHP, configure mTLS through `advanced_config['tls_config']`; client certificates and keys in a PHP stream context are not used for mTLS.
...
<Tabs syncKey="progLangInExamples">
  <TabItem label="Python">
    ...
    # Load PEM bytes from a secret store.
  ...
  <TabItem label="Java">
    ...
    // Load PEM bytes from a secret store.
    byte[] clientCert = "-----BEGIN CERTIFICATE-----\nMIIDXTCCAkWgode...".getBytes(StandardCharsets.UTF_8);
    byte[] clientKey = "-----BEGIN PRIVATE KEY-----\nMIIEvQIBAD...".getBytes(StandardCharsets.UTF_8);
  ...
  <TabItem label="Node">
    ...
    // Load PEM bytes from a secret store.
    const clientCertificate: Buffer = Buffer.from("-----BEGIN CERTIFICATE-----\nMIIDXTCCAkWgode...");
    const clientKey: Buffer = Buffer.from("-----BEGIN PRIVATE KEY-----\nMIIEvQIBAD...");
  ...
  <TabItem label="C#">
    ...
    // Load PEM bytes from a secret store.
    byte[] clientCert = Encoding.UTF8.GetBytes("-----BEGIN CERTIFICATE-----...-----END CERTIFICATE-----");
    byte[] clientKey = Encoding.UTF8.GetBytes("-----BEGIN PRIVATE KEY-----...-----END PRIVATE KEY-----");
  ...
  <TabItem label="PHP">
    ```php
    // Load PEM strings from a secret store.
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="C#">
    ...
    var config = new ClusterClientConfigurationBuilder()
        .WithAddress("address.example.com", 6379)
        .WithTls()
        .WithClientCertificate("/etc/glide/client.pem", "/etc/glide/client.key")
        .Build();
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="C#">
    ...
    // Read PEM-encoded certificate and key bytes from files.
    byte[] clientCert = File.ReadAllBytes("/etc/glide/client.pem");
    byte[] clientKey  = File.ReadAllBytes("/etc/glide/client.key");
  ...
  <TabItem label="PHP">
    ```php
    $clientCert = file_get_contents('/etc/glide/client.pem');
    $clientKey = file_get_contents('/etc/glide/client.key');
...
* **mTLS still needs TLS enabled at the top level.** The advanced TLS configuration only supplies the client-side material; without TLS on the base client configuration (`use_tls=True`, `.useTLS(true)`, `useTLS: true`, `.WithUseTLS(true)`, or `use_tls: true` in PHP), the client connects in plaintext and the mTLS material is not used.
