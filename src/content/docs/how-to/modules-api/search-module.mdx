...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="C#">
    ...
    await client.FtCreateAsync(index, schema,
        new FtCreateOptions { DataType = IndexDataType.Hash, Prefixes = [prefix] });

    // Insert hash data with vector fields (2-dimensional float32 vectors = 8 bytes each)
    byte[] vec0 = [0, 0, 0, 0, 0, 0, 0, 0];
    byte[] vec1 = [0, 0, 0, 0, 0, 0, 0x80, 0xBF];
    await client.HSetAsync(prefix + "0", [new("vec", vec0)]);
    await client.HSetAsync(prefix + "1", [new("vec", vec1)]);
    ...
    FtSearchResult result = await client.FtSearchAsync(index, query, searchOptions);

    Console.WriteLine($"Total results: {result.TotalResults}"); // Total results: 2
    foreach (FtSearchDocument doc in result.Documents)
        Console.WriteLine($"  {doc.Key}: {doc.Fields.Count} fields");
    ```
  </TabItem>
</Tabs>
