...
- **Blocking commands do not honor their timeout:** Blocking commands (`BLPOP key 0`, `BRPOP`, `XREAD BLOCK`, etc.) executed on a scoped connection currently time out at the client's configured request timeout instead of respecting the command's blocking timeout. This means `BLPOP key 0` (block forever) will fail after the request timeout expires. This will be fixed in a future release.
