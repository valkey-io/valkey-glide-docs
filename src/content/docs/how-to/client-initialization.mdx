...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="PHP">
    ...
    try {
        ...
        error_log("Failed to connect to cluster: " . $e->getMessage());
    }
    ```
  </TabItem>
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="PHP">
    Please refer to [the documentation within the code](https://github.com/valkey-io/valkey-glide-php/blob/main/valkey_glide_cluster.stub.php#L411-L419) for routing configuration.
  </TabItem>
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="PHP">
    Please refer to [the documentation within the code](https://github.com/valkey-io/valkey-glide-php/blob/main/valkey_glide_cluster.stub.php) for command return types.
  </TabItem>
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="PHP">
    ...
    try {
        ...
        error_log("Failed to connect to standalone server: " . $e->getMessage());
    }
    ```
  </TabItem>
...
### Configure node discovery in PHP

For standalone PHP clients, use `node_discovery_mode` to control how the client identifies the primary and replicas:

* `ValkeyGlide::NODE_DISCOVERY_MODE_STANDARD` is the default. It verifies node roles with `INFO REPLICATION` and uses only the addresses you provide.
* `ValkeyGlide::NODE_DISCOVERY_MODE_STATIC` trusts the provided addresses without checking their roles. The first address is treated as the primary. Use this mode when connecting through a proxy such as Envoy or HAProxy that does not support `INFO`.
* `ValkeyGlide::NODE_DISCOVERY_MODE_DISCOVER_ALL` discovers the primary and all replicas from any starting node.

To connect through a proxy without role discovery:
...
To discover the complete standalone topology from one node:

```php
$client = new ValkeyGlide();
$client->connect(
    addresses: [['host' => 'node.example.com', 'port' => 6379]],
    node_discovery_mode: ValkeyGlide::NODE_DISCOVERY_MODE_DISCOVER_ALL
);
```

`node_discovery_mode` applies only to `ValkeyGlide` standalone clients. When using `STATIC` mode through a proxy, do not set `client_name`.
