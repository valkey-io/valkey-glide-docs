...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Node">
    <Steps>
      ...
      2. Create a `Script` object with the Lua code.
      3. Execute the script using `invokeScript`.
    </Steps>
  ...
  <TabItem label="PHP">
    ...
    Use the native `eval()` or `evalsha()` commands instead.
    :::
    ...
    <Steps>
      ...
      2. Execute script with keys and arguments

         ```php
         $result = $client->eval(
             $lua,
             ['username', 'John Doe'],  // keys and args combined
             1  // num_keys - how many arguments are keys
         );
         echo $result; // username: John Doe
         ```
    </Steps>

    ### Using evalSha() for Better Performance

    For scripts executed multiple times, use `evalsha()` to avoid sending the script body repeatedly:
    ...
    <Steps>
      1. Load the script and get its SHA1 hash
         ...
         $sha = hash('sha1', $lua);
         // Or use SCRIPT LOAD to ensure it's cached on server
         ```

      2. Execute using the SHA1 hash
