...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Node">
    ...
    **With Routing (Cluster Mode)**
    ...
    ```typescript
    // Route to all nodes
    const allNodesResult = await clusterClient.fcallReadonlyWithRoute("get_value", [], { route: "allNodes" });

    // Route to all primary nodes
    const primaryResult = await clusterClient.fcallReadonlyWithRoute("get_value", [], { route: "allPrimaries" });

    // Route to a random node
    const randomResult = await clusterClient.fcallReadonlyWithRoute("get_value", [], { route: "randomNode" });
    ```
  </TabItem>
