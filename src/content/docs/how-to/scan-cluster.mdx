...
<Tabs syncKey="progLangExample">
  ...
  <TabItem label="Node">
    ...
    while (!cursor.isFinished()) {
        let keys;
        [cursor, keys] = await clusterClient.scan(cursor);
        allKeys.push(...keys);
    }
    ```
  </TabItem>
...
<Tabs syncKey="progLangExample">
  ...
  <TabItem label="Node">
    ...
    while (!cursor.isFinished()) {
        let keys;
        [cursor, keys] = await clusterClient.scan(cursor, { match: "*key*" });
...
<Tabs syncKey="progLangExample">
  <TabItem label="Python">
    ...
    # Returns around `count` keys: [b'my_key1']
    ```
  </TabItem>
  ...
  <TabItem label="Node">
    ...
    while (!cursor.isFinished()) {
        let keys;
        [cursor, keys] = await clusterClient.scan(cursor, { count: 10 });
        allKeys.push(...keys);
    }
...
<Tabs syncKey="progLangExample">
  ...
  <TabItem label="Node">
    ```typescript
    import { ObjectType } from "@valkey/valkey-glide";
    ...
    while (!cursor.isFinished()) {
        let keys;
        [cursor, keys] = await clusterClient.scan(cursor, { type: ObjectType.STRING });
