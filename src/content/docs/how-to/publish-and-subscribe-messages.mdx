...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Ruby">
    ```ruby
    publisher = Valkey.new(host: "localhost", port: 6379, protocol: :resp3)

    # Publish message on 'ch1' channel
    publisher.publish("Test message", "ch1")
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Ruby">
    ```ruby
    # Create a regular client with RESP3. No subscription configuration needed.
    client = Valkey.new(host: "localhost", port: 6379, protocol: :resp3)

    # --- Exact channel subscriptions ---

    # Blocking: waits up to 5000ms for server confirmation
    client.subscribe("news", "updates", timeout_ms: 5000)

    # Non-blocking (lazy): returns immediately, subscribes in background
    client.subscribe_lazy("alerts")

    # --- Pattern subscriptions ---

    # Blocking
    client.psubscribe("chat*", "event*", timeout_ms: 5000)

    # Non-blocking (lazy)
    client.psubscribe_lazy("log*")

    # --- Sharded subscriptions (cluster mode only) ---
    # cluster_client.ssubscribe("shard-ch1", timeout_ms: 5000)
    # cluster_client.ssubscribe_lazy("shard-ch2")

    # Retrieve messages via polling
    msg = client.get_pubsub_message
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Ruby">
    ```ruby
    # Define callback and context
    callback = ->(msg, ctx) { puts "Received #{msg.message}, context #{ctx}" }
    context = "example"

    # Configure the client to invoke the callback for messages published
    # to 'ch1' and 'ch2' and to channels matched by 'chat*' glob pattern.
    listening_client = Valkey.new(
      host: "localhost",
      port: 6379,
      protocol: :resp3,
      pubsub: {
        subscriptions: { exact: ["ch1", "ch2"], pattern: ["chat*"] },
        callback: callback,
        context: context
      }
    )
    ...
    listening_client.close    # Unsubscribe happens here
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Ruby">
    ...
    callback = lambda do |msg, _context|
      received.push(msg.message)
      puts "Received '#{msg.message}' on '#{msg.channel}'"
    end
    ...
    # Subscribe dynamically. Messages are delivered to the callback.
    client.subscribe("news", timeout_ms: 5000)

    # Publish a message (from another client)
    publishing_client.publish("Hello!", "news")
    sleep 0.5

    # Verify the callback received the message
    messages = Array.new(received.size) { received.pop }
    raise "Expected 'Hello!'" unless messages.include?("Hello!")
...
* **Non-blocking poll:** `tryGetPubSubMessage()` / `try_get_pubsub_message()` — returns the next message or `null`/`None`/`nil` immediately.
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Ruby">
    ...
    # Non-blocking: returns nil if no message is available
    message = listening_client.try_get_pubsub_message

    # Blocking: waits for the next message
    message = listening_client.get_pubsub_message
...
To unsubscribe from **all** channels/patterns of a given type, pass `None`/`null`/empty, or use the provided constants: `ALL_CHANNELS`, `ALL_PATTERNS`, `ALL_SHARDED_CHANNELS`. Ruby has no such constants. Call the method with no channels instead.
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Ruby">
    ```ruby
    # Unsubscribe from specific exact channels (blocking)
    client.unsubscribe("news", timeout_ms: 5000)

    # Unsubscribe from specific exact channels (lazy)
    client.unsubscribe_lazy("alerts")

    # Unsubscribe from all exact channels
    client.unsubscribe(timeout_ms: 5000)

    # Unsubscribe from specific patterns (blocking)
    client.punsubscribe("chat*", timeout_ms: 5000)

    # Unsubscribe from specific patterns (lazy)
    client.punsubscribe_lazy("log*")

    # Unsubscribe from all patterns
    client.punsubscribe(timeout_ms: 5000)

    # Sharded unsubscribe (cluster mode only)
    # cluster_client.sunsubscribe("shard-ch1", timeout_ms: 5000)
    # cluster_client.sunsubscribe_lazy("shard-ch2")
    # cluster_client.sunsubscribe(timeout_ms: 5000)
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Ruby">
    ...
    puts "Desired: #{state.desired_subscriptions}"
    puts "Actual: #{state.actual_subscriptions}"
    ```
  </TabItem>
</Tabs>
