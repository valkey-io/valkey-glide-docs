...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="C#">
    ...
    var config = new ClusterlientConfigurationBuilder()
        .WithAddress("localhost", 6379)
        .Build();
...
<Tabs syncKey="progLangInExamples">
  <TabItem label="Java">
    ...
    StandaloneSubscriptionConfiguration subscriptionsConfig = StandaloneSubscriptionConfiguration.builder()
  ...
  <TabItem label="Go">
    ...
    // Configure the client to invoke the callback for messages published
    // to 'ch1' and 'ch2' and to channels matched by 'chat*' glob pattern.
    subscriptionsConfig := NewStandaloneSubscriptionConfig().
    ...
    listeningClient, _ := NewGlideClient(NewGlideClientConfiguration().
        WithAddress(&NodeAddress{}).
        WithSubscriptionConfig(subscriptionsConfig))
