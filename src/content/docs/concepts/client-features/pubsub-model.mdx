...
*NOTE: Method naming varies by language (e.g., `subscribeLazy` in Java/Node.js/Go, `subscribe_lazy` in Python and Ruby). To unsubscribe from all channels/patterns, pass `None`/`null`/empty or use the constants `ALL_CHANNELS`, `ALL_PATTERNS`, `ALL_SHARDED_CHANNELS`.
