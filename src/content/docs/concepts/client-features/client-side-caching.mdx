...
| `serverAssisted`   | boolean           | `false` | When `true`, enables server-assisted invalidation using Valkey's `CLIENT TRACKING ON BCAST` (broadcast) mode — the only tracking mode GLIDE currently supports. Requires RESP3.|
...
### Diagnosing tracking state with `CLIENT TRACKINGINFO`
...
| Limitation | Details |
| --- | --- |
| **TTL-only expiration** | No server-side invalidation by default. Cached values may become stale if the key is modified on the server before the TTL expires. Java, Python, Node.js, and Go support [server-assisted invalidation](#server-assisted-invalidation) which eliminates this limitation. |
