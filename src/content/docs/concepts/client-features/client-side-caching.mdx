...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Java">
    ...
    ClientSideCache cache = ClientSideCache.builder()
        ...
        // .serverAssisted(true)             // Enable server-assisted invalidation (requires RESP3)
        .build();
...
| `serverAssisted`   | boolean           | `false` | When `true`, enables server-assisted invalidation via Valkey's `CLIENT TRACKING` protocol. Requires RESP3. Currently available in Java only. |
...
## Server-Assisted Invalidation (Java)

When `serverAssisted` is enabled, the Java client uses Valkey's [`CLIENT TRACKING`](https://valkey.io/commands/client-tracking/) protocol to receive push invalidation messages from the server whenever a cached key is modified. This eliminates the staleness window inherent in TTL-only mode — the server notifies the client immediately when data changes, and the client removes the stale entry from its local cache.
...
<Aside type="caution" title="RESP3 Required">
  Server-assisted caching requires the RESP3 protocol, which is enforced automatically by glide-core when `serverAssisted(true)` is set. Ensure your Valkey server supports RESP3 (Valkey 7.2+).
</Aside>

### Diagnosing tracking state with `clientTrackingInfo()`
...
// info contains:
//   "flags"    -> Set<String> e.g. {"on", "noloop"}
//   "redirect" -> Long (-1 if not redirecting)
//   "prefixes" -> Object[] of monitored key prefixes

// Cluster client — routes to a random node by default
Map<String, Object> clusterInfo = clusterClient.clientTrackingInfo().get();

// Cluster client — query all nodes
Map<String, Object> allNodesInfo = clusterClient.clientTrackingInfo(Route.ALL_NODES).get();
```

<Aside type="note" title="Java-only">
  Server-assisted invalidation is currently available in the Java client only. Support for Python, Node.js, Go, PHP, and C# will be added in future releases.
</Aside>
...
| Limitation | Details |
| --- | --- |
| **TTL-only expiration** | No server-side invalidation by default. Cached values may become stale if the key is modified on the server before the TTL expires. Java supports [server-assisted invalidation](#server-assisted-invalidation-java) which eliminates this limitation. |
