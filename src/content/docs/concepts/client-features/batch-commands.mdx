...
description: Learn about GLIDE batch operations.
---
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Node">
    ...
    // Create a non-atomic batch (pipeline)
    const nonAtomicBatch = new Batch(false)
    ```
  </TabItem>
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Node">
    ...
    const result = await clusterClient.exec(batch, false);
  ...
  <TabItem label="C#">
    ...
    {
        Console.WriteLine($"Batch execution aborted: {e.Message}");
    }
    // Output: Batch execution aborted: WRONGTYPE: Operation against a key holding the wrong kind of value
    ```
  </TabItem>
