...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Php">
    ```php
    // Create ValkeyGlide client with credentials
    ...
    // Clear the password from the connection configuration
    $client->clearConnectionPassword(); // Note: This will remove the password
