...
- **Connection Timeout**: During the initial connection establishment, the `connectionTimeout` setting (not `requestTimeout`) governs the timeout behavior (PHP uses `advanced_config['connection_timeout']`)
...
<Tabs>
  ...
  <TabItem syncKey="progLangInExamples" label="Php">
    ```php
    // Standalone client with lazy connect
