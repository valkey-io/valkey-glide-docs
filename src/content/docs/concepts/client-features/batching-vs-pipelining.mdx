...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Java">
    ...
    var response = client.exec(batch);
    ```
  </TabItem>
  <TabItem label="Node">
    ```typescript
    ...
    const response = await client.exec(batch);
    ```
  </TabItem>
  <TabItem label="Go">
    ```go
    ...
    res, err := client.Exec(context, transaction);
    ```
  </TabItem>

  <TabItem label="Php">
    ```php
    // Create atomic batch (transaction) using MULTI/EXEC
    // Default mode is ValkeyGlide::MULTI
...
<Tabs syncKey="progLangInExamples">
  <TabItem label="Python">
    ...
    transaction = Batch(is_atomic=False)
    transaction.set("key1", "value1")
  ...
  <TabItem label="Go">
    ...
    isAtomic := true;
    transaction := pipeline.NewStandaloneBatch(false)
        .Set("key1", "value1");
...
For detailed documentation, visit language-specific documentation:

* <a href={`/languages/python/valkey-commands/batch-transaction-and-pipelining`}> Python wrapper - Batch: Transaction and Pipelining </a>
* <a href={`/languages/nodejs/valkey-commands/batch-transaction-and-pipelining`}> NodeJS wrapper - Batch: Transaction and Pipelining </a>
* <a href={`/languages/java/valkey-commands/batch-transaction-and-pipelining`}> Java wrapper - Batch: Transaction and Pipelining </a>
* <a href={`/languages/go/valkey-commands/batch-transaction-and-pipelining`}> Golang wrapper - Batch: Transaction and Pipelining </a>
