...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="C#">
    ...
    // Create a simple script
    using var script = new Script("return 'Hello, Valkey!'");
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Node">
    ...
    const buyQuantity = "3";
    const script = new Script("return redis.call('DECRBY', KEYS[1], ARGV[1])");
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="C#">
    ...
    var result = await clusterClient.ScriptInvokeAsync(
        script,
        new ScriptOptions()
            .WithKeys("user:1000")   // Routed based on key hash
            .WithArgs("John"));
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Node">
    ```typescript
    const script = new Script("return redis.call('PING')");

    // Route to the node holding a specific key's slot
    const route = {type: "routeByAddress" as const, host: "localhost", port: 6379};
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="C#">
    ...
    batch.CustomCommand(new GlideString[] {
        "EVAL",
        "return redis.call('SET', KEYS[1], ARGV[1])",
        "1",           // Number of keys
        "script-key",  // Key
