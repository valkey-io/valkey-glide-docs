...
:::note
PHP GLIDE does not currently have dedicated Modules API support. However, you can use the `rawcommand()` method to execute module commands directly. See the [Custom Commands](/concepts/client-features/custom-commands) page for more information.
:::

## General Usage

Valkey Module commands are available through static calls that wrap the <a href="/concepts/client-features/custom-commands">custom command</a>.\
...
Once the module is loaded into the server, the following examples demonstrate basic behavior for calling module commands:
