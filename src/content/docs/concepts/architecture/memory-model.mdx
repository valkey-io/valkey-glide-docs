...
In a long-running test with **10 concurrent `GlideClient` instances** performing
continuous `GET` and `SET` operations against a cluster, steady-state memory
settled at approximately a **50/50 split** between JVM heap and native (Rust
core + JNI overhead + TCP buffers):

| Region | Approximate share |
|--------|-------------------|
| JVM heap (response objects, application state) | ~50% of RSS |
| Native (Rust runtime, connection buffers, inflight requests) | ~50% of RSS |

### Sizing guidance

| Scenario | Suggested `-Xmx` | Native headroom | Notes |
|----------|-------------------|-----------------|-------|
| 1–2 clients, light traffic | 70% of container | 30% | Minimal Rust footprint |
| Multiple clients, moderate traffic | 60% of container | 40% | More connections and inflight buffers |
| Many clients (10+), sustained high TPS | 50% of container | 50% | Matches observed steady-state ratio |
