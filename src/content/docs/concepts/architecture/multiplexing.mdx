...
To open a new connection, a new GLIDE client object should be created, as each GLIDE client maintains a single connection per node. You should open a new client for the following use cases:
...
For more on optmizing performance with multiple connections, take a look at our guide on <a href={`/concepts/client-features/batching-vs-pipelining`}>pipelining</a>.
