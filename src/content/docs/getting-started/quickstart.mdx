...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="C#">
    ...
    try
    {
        // Test the connection — PingAsync returns the round-trip latency
        TimeSpan latency = await client.PingAsync();

        Console.WriteLine($"Connected! Server latency: {latency.TotalMilliseconds}ms");
    }
    catch (Exception e)
    {
...
Running the above snippet should give you a response similar to:
...
:::note
The C# client's `PingAsync()` returns a `TimeSpan` representing the round-trip latency rather than the string `"PONG"`.
:::
