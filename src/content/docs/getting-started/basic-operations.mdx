---
title: Basic Operations
description: Learn fundamental Valkey GLIDE operations like setting, getting, and deleting keys
sidebar:
  order: 1

---

import { Aside } from '@astrojs/starlight/components';

import { Tabs, TabItem, LinkCard } from '@astrojs/starlight/components';

Now that you have a working connection, let's explore the fundamental operations you can perform with Valkey GLIDE.

## Understanding Client Commands

To communicate with Valkey, GLIDE clients provide methods for users to send commands to Valkey.
Under the hood, all clients share the same Rust core to communicate with Valkey. This
makes their behavior consistent between the different languages while still supporting language
specific features.

<Aside>
  To find more information on the Rust core that powers GLIDE clients, see [here](/concepts/architecture/rust-core-design/).
</Aside>

## Updating Keys

To update keys in Valkey, clients can send [SET](https://valkey.io/commands/set/) and [GET](https://valkey.io/commands/get/) commands.

<Tabs syncKey="progLangInExamples">
  <TabItem label="Python">
    ```python
    import asyncio
    from glide import GlideClient, GlideClientConfiguration, NodeAddress

    async def string_operations():
        config = GlideClientConfiguration(
            addresses=[NodeAddress("localhost", 6379)]
        )
        client = None

        try:
            client = await GlideClient.create(config)

            # Set a key-value pair
            await client.set("user:1000", "john_doe")
            print("✓ Key set successfully")
            
            # Get the value
            value = await client.get("user:1000")
            print(f"✓ Retrieved value: {value}")

        except Exception as e:
            print(f"Operation failed: {e}")

        finally:
            if client is not None:
                await client.close()

    if __name__ == "__main__":
        asyncio.run(string_operations())
    ```
  </TabItem>

  <TabItem label="Java">
    ```java
    import glide.api.GlideClient;
    import glide.api.models.configuration.GlideClientConfiguration;
    import glide.api.models.configuration.NodeAddress;
    import glide.api.models.commands.SetOptions;

    public class StringOperations {
        public static void main(String[] args) {
            GlideClientConfiguration config = GlideClientConfiguration.builder()
                .address(NodeAddress.builder().host("localhost").port(6379).build())
                .build();
            
            try (GlideClient client = GlideClient.createClient(config).get()) {
                // Set a key-value pair
                client.set("user:1000", "john_doe").get();
                System.out.println("✓ Key set successfully");
                
                // Get the value
                String value = client.get("user:1000").get();
                System.out.println("✓ Retrieved value: " + value);

            } catch (Exception e) {
                System.err.println("Error: " + e.getMessage());
            }
        }
    }
    ```
  </TabItem>

  <TabItem label="Node">
    ```javascript
    const { GlideClient } = require('@valkey/valkey-glide');

    async function stringOperations() {
        let client;

        try {
            client = await GlideClient.createClient({
                addresses: [{ host: 'localhost', port: 6379 }]
            });
            // Set a key-value pair
            await client.set('user:1000', 'john_doe');
            console.log('✓ Key set successfully');
            
            // Get the value
            const value = await client.get('user:1000');
            console.log(`✓ Retrieved value: ${value}`);

        } catch (error) {
            console.error(`Operation failed: ${error.message}`);
        } finally {
            if (client) {
                client.close();
            }
        }
    }

    stringOperations().catch(console.error);
    ```
  </TabItem>

  <TabItem label="Go">
    ```go
    package main
    import (
        "context"
        "fmt"
        "log"

        glide "github.com/valkey-io/valkey-glide/go/v2"
        "github.com/valkey-io/valkey-glide/go/v2/config"
    )

    func main() {
        config := config.NewClientConfiguration().
            WithAddress(&config.NodeAddress{Host: "localhost", Port: 6379})

        client, err := glide.NewClient(config)
        if err != nil {
            log.Fatalf("NewClient failed: %v", err)
        }
        defer client.Close()

        // Set a key-value pair
        _, err = client.Set(context.Background(), "user:1000", "john_doe")
        if err != nil {
            log.Printf("Set failed: %v", err)
            return
        }
        fmt.Println("✓ Key set successfully")

        // Get the value
        value, err := client.Get(context.Background(), "user:1000")
        if err != nil {
            log.Printf("Get failed: %v", err)
            return
        }
        fmt.Printf("✓ Retrieved value: %v\n", value)
    }
    ```
  </TabItem>

  <TabItem label="PHP">
    ```php
    // Create ValkeyGlide client
    $client = new ValkeyGlide();

    try {
        $client->connect(addresses: [['host' => 'localhost', 'port' => 6379]]);

        // Set a key-value pair
        $client->set('user:1000', 'john_doe');
        echo "✓ Key set successfully\n";
        
        // Get the value
        $value = $client->get('user:1000');
        echo "✓ Retrieved value: {$value}\n";

    } catch (ValkeyGlideException $e) {
        echo "Operation failed: {$e->getMessage()}\n";
    } finally {
        $client->close();
    }
    ```
  </TabItem>

  <TabItem label="C#">
    ```csharp
    using Valkey.Glide;
    using static Valkey.Glide.ConnectionConfiguration;

    var config = new StandaloneClientConfigurationBuilder().Build();
    try
    {
        await using var client = await GlideClient.CreateClient(config);

        // Set a key-value pair
        await client.SetAsync("user:1000", "john_doe");
        Console.WriteLine("✓ Key set successfully");

        // Get the value
        ValkeyValue value = await client.GetAsync("user:1000");
        Console.WriteLine($"✓ Retrieved value: {value}");
    }
    catch (Exception e)
    {
        Console.WriteLine($"Operation failed: {e.Message}");
    }
    ```
  </TabItem>

  <TabItem label="Ruby">
    ```ruby
    require "valkey"

    client = nil

    begin
      client = Valkey.new(host: "localhost", port: 6379)

      # Set a key-value pair
      client.set("user:1000", "john_doe")
      puts "✓ Key set successfully"

      # Get the value
      value = client.get("user:1000")
      puts "✓ Retrieved value: #{value}"
    rescue StandardError => e
      puts "Operation failed: #{e.message}"
    ensure
      client&.close
    end
    ```
  </TabItem>
</Tabs>

The remaining examples assume that `client` was created with the error handling and cleanup pattern shown above.

## Making Multiple Atomic Operations

Clients can send [MSET](https://valkey.io/commands/mset/) and [MGET](https://valkey.io/commands/mget/) commands, which are used to set/get keys and values atomically.

<Tabs syncKey="progLangInExamples">
  <TabItem label="Python">
    ```python
    # Set multiple keys at once
    await client.mset({"user:1001": "alice", "user:1002": "bob", "user:1003": "charlie"})
    print("✓ Multiple keys set")

    # Get multiple keys at once
    values = await client.mget(["user:1001", "user:1002", "user:1003"])
    print(f"✓ Retrieved values: {values}")
    ```
  </TabItem>

  <TabItem label="Java">
    ```java
    // Set multiple keys at once
    Map<String, String> keyValues = Map.of(
        "user:1001", "alice",
        "user:1002", "bob", 
        "user:1003", "charlie"
    );
    client.mset(keyValues).get();
    System.out.println("✓ Multiple keys set");

    // Get multiple keys at once
    String[] keys = {"user:1001", "user:1002", "user:1003"};
    String[] values = client.mget(keys).get();
    System.out.println("✓ Retrieved values: " + Arrays.toString(values));
    ```
  </TabItem>

  <TabItem label="Node">
    ```javascript
    // Set multiple keys at once
    await client.mset({
        'user:1001': 'alice',
        'user:1002': 'bob',
        'user:1003': 'charlie'
    });
    console.log('✓ Multiple keys set');

    // Get multiple keys at once
    const values = await client.mget(['user:1001', 'user:1002', 'user:1003']);
    console.log(`✓ Retrieved values: ${values}`);
    ```
  </TabItem>

  <TabItem label="Go">
    ```go
    // Set multiple keys at once
    keyValues := map[string]string{
        "user:1001": "alice",
        "user:1002": "bob",
        "user:1003": "charlie",
    }
    _, err = client.MSet(context.Background(), keyValues)
    if err != nil {
        log.Fatalf("MSet failed: %v", err)
    }
    fmt.Println("✓ Multiple keys set")

    // Get multiple keys at once
    keys := []string{"user:1001", "user:1002", "user:1003"}
    values, err := client.MGet(context.Background(), keys)
    if err != nil {
        log.Fatalf("MGet failed: %v", err)
    }
    fmt.Printf("✓ Retrieved values: %v\n", values)
    ```
  </TabItem>

  <TabItem label="PHP">
    ```php
    // Set multiple keys at once
    $client->mset([
        'user:1001' => 'alice',
        'user:1002' => 'bob',
        'user:1003' => 'charlie'
    ]);
    echo "✓ Multiple keys set\n";

    // Get multiple keys at once
    $values = $client->mget(['user:1001', 'user:1002', 'user:1003']);
    echo "✓ Retrieved values: " . implode(', ', $values) . "\n";
    ```
  </TabItem>

  <TabItem label="C#">
    ```csharp
    using Valkey.Glide;

    // Set multiple keys at once
    await client.SetAsync(new KeyValuePair<ValkeyKey, ValkeyValue>[]
    {
        new("user:1001", "alice"),
        new("user:1002", "bob"),
        new("user:1003", "charlie"),
    });
    Console.WriteLine("✓ Multiple keys set");

    // Get multiple keys at once
    var values = await client.GetAsync(["user:1001", "user:1002", "user:1003"]);
    Console.WriteLine($"✓ Retrieved values: {string.Join(", ", values)}");
    ```
  </TabItem>

  <TabItem label="Ruby">
    ```ruby
    # Set multiple keys at once
    client.mapped_mset("user:1001" => "alice", "user:1002" => "bob", "user:1003" => "charlie")
    puts "✓ Multiple keys set"

    # Get multiple keys at once
    values = client.mget("user:1001", "user:1002", "user:1003")
    puts "✓ Retrieved values: #{values}"
    ```
  </TabItem>
</Tabs>

## Removing Keys

Clients can also send [DEL](https://valkey.io/commands/del/) commands to delete a list of keys.

<Tabs syncKey="progLangInExamples">
  <TabItem label="Python">
    ```python
    deleted_count = await client.delete(["user:1001", "user:1002", "user:1003"])
    print(f"✓ Deleted {deleted_count} keys")
    ```
  </TabItem>

  <TabItem label="Java">
    ```java
    String[] keys = {"user:1001", "user:1002", "user:1003"};
    Long deletedCount = client.del(keys).get();
    System.out.println("✓ Deleted " + deletedCount + " keys");
    ```
  </TabItem>

  <TabItem label="Node">
    ```javascript
    const deletedCount = await client.del(['user:1001', 'user:1002', 'user:1003']);
    console.log(`✓ Deleted ${deletedCount} keys`);
    ```
  </TabItem>

  <TabItem label="Go">
    ```go
    keys := []string{"user:1001", "user:1002", "user:1003"}
    deletedCount, err := client.Del(context.Background(), keys)
    if err != nil {
        log.Fatalf("Del failed: %v", err)
    }
    fmt.Printf("✓ Deleted %d keys\n", deletedCount)
    ```
  </TabItem>

  <TabItem label="PHP">
    ```php
    $deletedCount = $client->del(['user:1001', 'user:1002', 'user:1003']);
    echo "✓ Deleted {$deletedCount} keys\n";
    ```
  </TabItem>

  <TabItem label="C#">
    ```csharp
    using Valkey.Glide;

    var count = await client.DeleteAsync(["user:1001", "user:1002", "user:1003"]);
    Console.WriteLine($"✓ Deleted {count} keys");
    ```
  </TabItem>

  <TabItem label="Ruby">
    ```ruby
    deleted_count = client.del("user:1001", "user:1002", "user:1003")
    puts "✓ Deleted #{deleted_count} keys"
    ```
  </TabItem>
</Tabs>

## What's Next

Now that you understand the basics, take a look at some other topics on Valkey GLIDE:

<LinkCard title="GLIDE's Rust Core" href={`/concepts/architecture/rust-core-design`} description="Learn more about the underlying engine that connects GLIDE drivers." />

<LinkCard title="Supported Commands" href={`/commands/supported-commands`} description="See which commands are supported by GLIDE clients." />
