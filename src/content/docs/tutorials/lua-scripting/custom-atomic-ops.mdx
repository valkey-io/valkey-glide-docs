...
One of the advantages of using Lua scripts is its atomicity which guarantees script executions are all-or-nothing.
...
try to read the same object to then modify it.

## Using Valkey Commands

To avoid a race condition, a client will need to ensure that the item\_key has not changed while it is updating the item\_key.
...
in high contention situations; Many clients are trying to buy the same item. Any client that fails to modify `item_key` successfully must retry until they do.
...
The following Lua script implements our "purchase" logic. It checks for `current_stock >= requested_quantity`:
...
When executed, this script is guaranteed to be all-or-nothing.
...
will raise it to the user, stopping the script immediately.
...
Valkey provides `KEYS` and `ARGV` to handle input parameters, each with their own specific purpose:
...
Note that both `KEYS` and `ARGV` parameters must be strings.
...
The following is an example implementation for a client that handles inventory deduction atomically using Lua scripting.
It will:
